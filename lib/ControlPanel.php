    ...
    public function getWebForm($addRecord = false)
    {
        ...
        foreach ($this->_tables as $tableName => $tableData) {
            foreach ($tableData['fields'] as $fieldName => $fieldData) {
                if (! isset($fieldData['section']) || (is_array($fieldData['section']) || $fieldData['section'] instanceof \Countable ? count($fieldData['section']) : 0) == 0) {
                    continue;
                }
                if ($fieldData['section'][0] == CP_LISTVIEW && (is_array($fieldData['section']) || $fieldData['section'] instanceof \Countable ? count($fieldData['section']) : 0) == 1) {
                    continue;
                }
                $this->_wf->addField(
